 ...
 *	    andip71 for this module in Boeffla Kernel
 *
...
#include "mdnie.h"
#include "mdnie_table_u1_samsung.h"
#include "mdnie_table_u1_hardcore.h"
					...
					const char *buf, size_t count)
{
	...
	if ((val >= 0) && (val <= 1))
	{
		...
		if (mdnie_preset == 0) // original Samsung mdnie settings
		{
			...
			update_mdnie_array("tune_dynamic_ui", tune_dynamic_ui_sm);
			update_mdnie_array("tune_dynamic_video", tune_dynamic_video_sm);
			update_mdnie_array("tune_dynamic_vt", tune_dynamic_vt_sm);
			update_mdnie_array("tune_movie_gallery", tune_movie_gallery_sm);
			update_mdnie_array("tune_movie_ui", tune_movie_ui_sm);
			update_mdnie_array("tune_movie_video", tune_movie_video_sm);
			update_mdnie_array("tune_movie_vt", tune_movie_vt_sm);
			update_mdnie_array("tune_standard_gallery", tune_standard_gallery_sm);
			update_mdnie_array("tune_standard_ui", tune_standard_ui_sm);
			update_mdnie_array("tune_standard_video", tune_standard_video_sm);
			update_mdnie_array("tune_standard_vt", tune_standard_vt_sm);
			update_mdnie_array("tune_natural_gallery", tune_natural_gallery_sm);
			...
			update_mdnie_array("tune_warm", tune_warm_sm);
			update_mdnie_array("tune_warm_outdoor", tune_warm_outdoor_sm);

			// finally write ui preset, based on current scenario to mdnie controller
			write_mdnie_ui("tune_standard_ui", tune_dynamic_ui_sm, tune_movie_ui_sm,
							tune_standard_ui_sm, tune_natural_ui_sm);
		...
		if (mdnie_preset == 1) // Hardcore speedmod mdnie settings
		{
			...
			update_mdnie_array("tune_dynamic_ui", tune_dynamic_ui_hr);
			update_mdnie_array("tune_dynamic_video", tune_dynamic_video_hr);
			update_mdnie_array("tune_dynamic_vt", tune_dynamic_vt_hr);
			update_mdnie_array("tune_movie_gallery", tune_movie_gallery_hr);
			update_mdnie_array("tune_movie_ui", tune_movie_ui_hr);
			update_mdnie_array("tune_movie_video", tune_movie_video_hr);
			update_mdnie_array("tune_movie_vt", tune_movie_vt_hr);
			update_mdnie_array("tune_standard_gallery", tune_standard_gallery_hr);
			update_mdnie_array("tune_standard_ui", tune_standard_ui_hr);
			update_mdnie_array("tune_standard_video", tune_standard_video_hr);
			update_mdnie_array("tune_standard_vt", tune_standard_vt_hr);
			update_mdnie_array("tune_natural_gallery", tune_natural_gallery_hr);
			update_mdnie_array("tune_natural_ui", tune_natural_ui_hr);
			update_mdnie_array("tune_natural_video", tune_natural_video_hr);
			update_mdnie_array("tune_natural_vt", tune_natural_vt_hr);
			update_mdnie_array("tune_camera", tune_camera_hr);
			update_mdnie_array("tune_camera_outdoor", tune_camera_outdoor_hr);
			update_mdnie_array("tune_cold", tune_cold_hr);
			update_mdnie_array("tune_cold_outdoor", tune_cold_outdoor_hr);
			update_mdnie_array("tune_normal_outdoor", tune_normal_outdoor_hr);
			update_mdnie_array("tune_warm", tune_warm_hr);
			update_mdnie_array("tune_warm_outdoor", tune_warm_outdoor_hr);

			// finally write ui preset, based on current scenario to mdnie controller
			write_mdnie_ui("tune_standard_ui", tune_dynamic_ui_hr, tune_movie_ui_hr,
...
static int mdnie_preset_init(void)
{
	...
	if (sysfs_create_group(&mdnie_preset_control_device.this_device->kobj,
		...
		printk("Apolo-kernel: failed to create sys fs object.\n");
		return 0;
	}

	// Print debug info
	printk("Apolo-kernel: mdnie control device started\n");
...
static void mdnie_preset_exit(void)
{
	...
	// Print debug info
	printk("Apolo-kernel: mdnie control device stopped\n");
}
