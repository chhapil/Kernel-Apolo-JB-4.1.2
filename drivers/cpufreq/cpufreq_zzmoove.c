 ...
 *
 * Tweaked by pedestre for Apolo Kernel :) Samsung S2 i9100
 * --------------------------------------------------------------------------------------------------------------------------------------------------------
...
#define DEF_FREQUENCY_UP_THRESHOLD		(85) // -  68 original
#define DEF_FREQUENCY_UP_THRESHOLD_HOTPLUG1	(85)	// ZZ: default for hotplug up threshold for cpu1 (cpu0 stays allways on)  -  68 original
#define DEF_FREQUENCY_UP_THRESHOLD_HOTPLUG2	(0)	// ZZ: default for hotplug up threshold for cpu2 (cpu0 stays allways on)
#define DEF_FREQUENCY_UP_THRESHOLD_HOTPLUG3	(0)	// ZZ: default for hotplug up threshold for cpu3 (cpu0 stays allways on)
#define DEF_FREQUENCY_DOWN_THRESHOLD		(80)    // 52 original
#define DEF_FREQUENCY_DOWN_THRESHOLD_HOTPLUG1	(75)	// ZZ: default for hotplug down threshold for cpu1 (cpu0 stays allways on) -  55 original
#define DEF_FREQUENCY_DOWN_THRESHOLD_HOTPLUG2	(0)	// ZZ: default for hotplug down threshold for cpu2 (cpu0 stays allways on)
#define DEF_FREQUENCY_DOWN_THRESHOLD_HOTPLUG3	(0)	// ZZ: default for hotplug down threshold for cpu3 (cpu0 stays allways on)
...
#define DEF_UP_THRESHOLD_SLEEP			(95)	// ZZ: default for tuneable up_threshold_sleep -  90 original
#define DEF_DOWN_THRESHOLD_SLEEP		(85)	// ZZ: default for tuneable down_threshold_sleep -  44 original
...
#define DEF_HOTPLUG_SLEEP			(1)	// ZZ: default for tuneable hotplug_sleep -  0 original (todos los cores online)

/*
* ZZ: Frequency Limit: 0 do not limit frequency and use the full range up to cpufreq->max limit
* values 200000 -> 1200000 (khz)  //-  1600000 original
*/

#define DEF_FREQ_LIMIT				(1200000)	// ZZ: default for tuneable freq_limit -  0 original
#define DEF_FREQ_LIMIT_SLEEP			(500000)	// ZZ: default for tuneable freq_limit_sleep -  0 original
...
//Primera columna (0) frecuencia actual
//Segunda columna(1) frecuencia a la que sube si load < smooth_up
//Tercera columna(2) frecuencia a la que baja si load < smooth_up
static int mn_freqs[5][5]={
    {1200000,1200000, 800000,1200000,1000000},
    {1000000,1200000, 800000,1200000, 800000},
    { 800000,1000000, 500000,1000000, 500000},
    { 500000, 800000, 200000, 800000, 200000},
...
/*
 * Table modified for use with Samsung I9300 by ZaneZam November 2012
 * zzmoove v0.3 - table modified to reach overclocking frequencies up to 1200mhz
 * zzmoove v0.4 - added fast scaling columns to frequency table
 */
...
//Primera columna (0) frecuencia actual. Se mira y se busca en las otras columnas la frecuencia de subida o bajada
//Segunda columna(1) frecuencia a la que sube si load >= smooth_up
//Tercera columna(2) frecuencia a la que baja si load >= smooth_up
static int mn_freqs_power[5][5]={
    {1200000,1200000,1000000,1200000,1000000},
    {1000000,1200000, 800000,1200000, 800000},
    { 800000,1000000, 500000,1000000, 500000},
    { 500000, 800000, 200000, 800000, 200000},
    { 200000, 500000, 200000, 500000, 200000}
...
static int mn_get_next_freq(unsigned int curfreq, unsigned int updown, unsigned int load) {
    ...
    {
	    for(i = 0; i < 5 ; i++) //i<15 inicial (dimension de la tabla)
    	    {
...
/* 
 * ZZ: added tuneable -> possible values: 0 disable, 200000-1200000 khz -> freqency soft-limit, if not set default is 0
					  ...
					  const char *buf, size_t count)
{
	...
	if (ret != 1 || input > 1200000 || (input < 200000 && input != 0))
		return -EINVAL;
...
/* 
 * ZZ: added tuneable -> possible values: 0 disable, 200000-1200000 khz -> freqency soft-limit on early suspend, if not set default is 0
 */
